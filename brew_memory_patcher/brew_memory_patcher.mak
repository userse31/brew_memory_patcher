...
# indicate a module must be built and named brew_memory_patcher.dll
BUILD_BREWMODS = brew_memory_patcher

# specifies all the C source files that need to be compiled
brew_memory_patcher_C_SRCS = AEEAppGen \
	AEEModGen

# specifies all the C++ source files that need to be compiled
brew_memory_patcher_CPP_SRCS = Brew_Memory_Patcher

# specifies the cif files to be compiled
brew_memory_patcher_CIFS = brew_memory_patcher

# specifies the resource files to be compiled
brew_memory_patcher_CARS = brew_memory_patcher
...
brew_memory_patcher_C_SRCS +=glibc_stubs
endif

ifneq ($(V_TARGET),Win32)
ifneq ($(V_TARGET),mingw)
brew_memory_patcher_C_SRCS +=mod_malloc
endif
endif
...
BREWMP_INPUT = Debug/Brew_Memory_Patcher.dll \
	Debug/brew_memory_patcher.bar \
	Debug/Brew_Memory_Patcher.mif

endif

ifeq ($(V),Win32_Release)
#Win32_Release package settings
BREWMP_INPUT = Release/Brew_Memory_Patcher.dll \
	Release/brew_memory_patcher.bar \
	Release/Brew_Memory_Patcher.mif

endif

ifneq ($(V_TARGET),Win32)
#Arm package settings
BREWMP_INPUT = $(OBJ_DIR)/Brew_Memory_Patcher.mod \
	$(OBJ_DIR)/brew_memory_patcher.bar \
	$(OBJ_DIR)/Brew_Memory_Patcher.mif
